...
$productchange = isset($_POST['product_change']) ? intval($_POST['product_change']) : "3";
...
//總共有幾筆資料
$t_sql = "SELECT count(1) FROM `product_total` WHERE product_category_id = {$productchange}";
...
if ($totalRows > 0) {
    ...
    //要顯示的欄位 
    $sql = sprintf("SELECT pt.product_id, pt.product_name, pt.product_price, pt.product_pic, pt.brand_category_id, pc.product_category, bc.brand_category, pt.product_category_id, pt.created_at, pt.updated_at FROM `product_total` pt JOIN product pc ON pc.product_category_id = pt.product_category_id JOIN brand bc ON pt.brand_category_id = bc.brand_category_id WHERE pt.product_category_id = {$productchange} LIMIT %s, %s;", $perPage * ($page - 1), $perPage);
...
<div class="container mb-5">
    <div class="row justify-content-center mb-5">
        <div class="col-4">
            <h2 class="text-center"><a style="color:#4a493b;text-decoration:none">商品分類管理頁</a></h2>
        </div>
    </div>
    <div class="row">
